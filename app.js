...
function sleepPage(){
  setHeader('Sonno','Recupero reale prima del programma');const keys=Object.keys(state.sleepLogs).sort().reverse().slice(0,10);
  app.innerHTML=`<div class="stack"><button class="btn ghost smallbtn" onclick="backMore()">‹ Altro</button><section class="card"><h3>Sonno recente</h3>${keys.length?keys.map(k=>`<div class="history-item"><div class="row"><b>${fmtDate(k,{weekday:'short',day:'numeric',month:'short'})}</b><span>${state.sleepLogs[k].hours||'–'} h · qualità ${state.sleepLogs[k].quality||'–'}/5</span></div></div>`).join(''):'<p class="muted small">Nessun dato ancora. Compila il check-in quotidiano.</p>'}<button class="btn" onclick="openCheckin('${dateKey(TODAY())}')">Check-in di oggi</button></section><section class="notice">Nei giorni dopo notte/24h il sonno ha priorità sulla seduta prevista: SmartCoach non prova a “recuperare” tutto comprimendo gli allenamenti.</section></div>`;
}

function connectionsPage(){
  setHeader('Connessioni','Snapshot reali + limiti della PWA');
  app.innerHTML=`<div class="stack"><button class="btn ghost smallbtn" onclick="backMore()">‹ Altro</button><section class="card"><div class="row"><div><h3>COROS</h3><span class="pill good">snapshot ${fmtDate(state.coros.synced)}</span></div><b class="accent">✓</b></div><div class="grid2" style="margin-top:12px"><div class="metric"><small>VO₂max</small><b>${state.coros.vo2max}</b></div><div class="metric"><small>Soglia</small><b>${state.coros.thresholdPace}</b></div><div class="metric"><small>Recovery</small><b>${state.coros.recovery}%</b></div><div class="metric"><small>HRV baseline</small><b>${state.coros.hrvBaseline} ms</b></div><div class="metric"><small>RHR</small><b>${state.coros.restingHr} bpm</b></div><div class="metric"><small>Load S/L</small><b>${state.coros.shortLoad}/${state.coros.longLoad}</b></div></div></section>
  <section class="card"><div class="row"><div><h3>Google Calendar</h3><span class="pill good">turni importati</span></div><b class="accent">✓</b></div><p class="muted small">Snapshot dei turni 25–30 settembre già usato dall'algoritmo di adattamento.</p></section>
  <section class="card"><div class="row"><div><h3>Apple Health / Bevel</h3><span class="pill warn">manuale</span></div><b class="warn">!</b></div><p class="muted small">Una PWA nel browser non può leggere direttamente HealthKit. HRV, sonno, RHR e altri dati si possono inserire nel check-in; la struttura dell'app è già pronta per una futura integrazione nativa/OAuth.</p></section>
  <section class="notice">COROS e Calendar qui sono uno <strong>snapshot</strong>, non una sincronizzazione live dal browser. Quando mi chiedi di aggiornare SmartCoach in chat, posso rileggere i dati collegati e aggiornare il progetto senza cambiare lo shortcut.</section>
  <button class="btn secondary" onclick="copySyncPrompt()">Copia richiesta di sincronizzazione</button></div>`;
}
async function copySyncPrompt(){const txt='Aggiorna SmartCoach con gli ultimi dati COROS, recovery/training load e i prossimi turni del mio Google Calendar, poi adatta il programma.';try{await navigator.clipboard.writeText(txt);toast('Testo copiato')}catch{toast(txt)}}

function dataPage(){
  setHeader('Dati','Backup locale del tuo SmartCoach');
  app.innerHTML=`<div class="stack"><button class="btn ghost smallbtn" onclick="backMore()">‹ Altro</button><section class="card"><h3>Backup</h3><p class="muted small">Allenamenti, readiness, peso e diario restano sul dispositivo in localStorage.</p><div class="actions"><button class="btn" onclick="exportData()">Esporta JSON</button><button class="btn secondary" onclick="document.getElementById('importFile').click()">Importa</button></div><input id="importFile" class="hidden" type="file" accept="application/json" onchange="importData(this.files[0])"></section><section class="card"><h3>Versione</h3><p class="muted small">SmartCoach ${APP_VERSION} · PWA offline · dati locali.</p><button class="btn danger" onclick="resetData()">Azzera dati locali</button></section></div>`;
}
function exportData(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`smartcoach-backup-${dateKey(TODAY())}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);toast('Backup esportato')}
function importData(file){if(!file)return;const r=new FileReader();r.onload=()=>{try{state=migrate(JSON.parse(r.result));save();toast('Backup importato');dataPage()}catch{toast('File non valido')}};r.readAsText(file)}
function resetData(){if(!confirm('Azzero tutti i dati locali di SmartCoach su questo dispositivo?'))return;state=defaultState();save();toast('Dati azzerati');todayView(dateKey(TODAY()))}

function installHint(){
  const standalone=window.matchMedia('(display-mode: standalone)').matches||navigator.standalone===true;
  if(standalone)return;
  const seen=sessionStorage.getItem('installHint');if(seen)return;sessionStorage.setItem('installHint','1');
  setTimeout(()=>toast('Su iPhone: Safari → Condividi → Aggiungi alla Home'),700);
}

function nav(tab){if(tab==='today')todayView(state.selectedDate||dateKey(TODAY()));else if(tab==='week')weekView();else if(tab==='train')trainingHub();else if(tab==='trend')trendView();else moreHome()}
document.querySelectorAll('.bottom-nav button').forEach(b=>b.addEventListener('click',()=>nav(b.dataset.tab)));
document.querySelector('#quickCheck').addEventListener('click',()=>openCheckin(state.selectedDate||dateKey(TODAY())));
...
window.addEventListener('offline',()=>toast('Offline · dati locali disponibili'));

window.startRecommended=startRecommended;window.openCheckin=openCheckin;window.saveCheckin=saveCheckin;window.todayView=todayView;window.setPlace=setPlace;window.openSession=openSession;window.changeWorkoutPlace=changeWorkoutPlace;window.swapExercise=swapExercise;window.saveStrength=saveStrength;window.startTimer=startTimer;window.saveCardio=saveCardio;window.saveRecovery=saveRecovery;window.deleteHistory=deleteHistory;window.moreView=moreView;window.backMore=backMore;window.saveBody=saveBody;window.saveBaseline=saveBaseline;window.saveNutrition=saveNutrition;window.copySyncPrompt=copySyncPrompt;window.exportData=exportData;window.importData=importData;window.resetData=resetData;

todayView(dateKey(TODAY()));installHint();
