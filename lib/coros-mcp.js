...
export class BlobOAuthProvider{
  ...
  tokens(ctx){
    if(ctx?.issuer)return this.store.tokensByIssuer?.[ctx.issuer];
    return this.store.tokens;
  }
  async saveTokens(tokens,ctx){
    this.store.tokens=tokens;
    if(ctx?.issuer)this.store.tokensByIssuer={...(this.store.tokensByIssuer||{}),[ctx.issuer]:tokens};
    this.store.tokenSavedAt=new Date().toISOString();
    await saveOAuth(this.store);
  }
  async state(){
  ...
  async saveResourceUrl(v){this.store.resourceUrl=v;await saveOAuth(this.store)}
  resourceUrl(){return this.store.resourceUrl}
...
export async function resetOAuthForFreshAuthorization({keepRegional=true}={}){
  const old=await loadOAuth();
  const regional=keepRegional?trustedCorosMcpUrl(old?.mcpUrl):null;
  const fresh={};
  if(regional)fresh.mcpUrl=String(regional);
  await saveOAuth(fresh);
  return fresh;
}
export async function makeProvider(req,{fresh=false}={}){
  const store=fresh?await resetOAuthForFreshAuthorization({keepRegional:true}):await loadOAuth();
...
export function decodeToolText(value){
  const s=String(value??'').trim();
  if(!s)return '';
  // COROS MCP currently wraps tool text as a JSON string, e.g. "\"line 1\\nline 2\"".
  // Decode only quoted JSON strings; leave ordinary text and structured JSON untouched.
...
export function resultText(r){
  if(!r)return '';
  if(typeof r==='string')return decodeToolText(r);
  if(Array.isArray(r.content)){
    return r.content.filter(x=>x?.type==='text').map(x=>decodeToolText(x.text||'')).join('\n');
  }
  if(r.text)return decodeToolText(r.text);
...
function hmMinutes(s){if(!s)return null;const h=Number((s.match(/(\d+)\s*h/)||[])[1]||0),m=Number((s.match(/(\d+)\s*min/)||[])[1]||0);return h*60+m}
...
export function parseActivityDetail(text){
  return {
    trainingLoad:val(text,/Training Load:\s*([\d.]+)/i)===null?null:Number(val(text,/Training Load:\s*([\d.]+)/i)),
    aerobicTE:val(text,/Aerobic TE:\s*([\d.]+)/i)===null?null:Number(val(text,/Aerobic TE:\s*([\d.]+)/i)),
    anaerobicTE:val(text,/Anaerobic TE:\s*([\d.]+)/i)===null?null:Number(val(text,/Anaerobic TE:\s*([\d.]+)/i)),
